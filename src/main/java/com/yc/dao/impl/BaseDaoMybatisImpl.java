...
public class BaseDaoMybatisImpl<T> implements BaseDao<T> {
	...
	public double getFunc(Class<T> clazz, String sqlId,Map<String, Object> parameterMap) {
		return new Double(sqlSession.selectOne(mapperPath+clazz.getSimpleName()+"Mapper."+sqlId,parameterMap).toString());
	}

	@Override
	public T findOne(Class<T> clazz, String sqlId, Integer uid) {
		List<T> list = sqlSession.selectList(mapperPath+clazz.getSimpleName()+"Mapper."+sqlId,uid);
		if(list != null && list.size()>0) {
			return list.get(0);
		} else {
			return null;
	...
	@Override
	public House findOne(Class<House> clazz, Integer hid, String sqlId) {
		return  sqlSession.selectOne(mapperPath+clazz.getSimpleName()+"Mapper."+sqlId,hid);
